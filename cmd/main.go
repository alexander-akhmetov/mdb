...
func main() {
	...
	performanceMode := flag.Bool("performance", false, "Run performance test")
	flag.BoolVar(performanceMode, "p", false, "Run performance test")

	performanceMaxKeysHelp := "performance: maximum keys to insert"
	performanceMaxKeys := flag.Int("performance-max-keys", -1, performanceMaxKeysHelp)
	flag.IntVar(performanceMaxKeys, "k", -1, performanceMaxKeysHelp)

	checkKeys := flag.Bool("check-keys", false, "performance: check inserted keys")
	flag.BoolVar(checkKeys, "c", false, "performance: check inserted keys")
	...
	if *performanceMode {
		...
		defer db.Stop()
		performanceTest(db, *performanceMaxKeys, *checkKeys)
		return
	}
