...
// now it's better to start perfomance test with external output filtering:
//
// go run db/*.go -p 2>&1 |  grep -v 'DEBUG'
//
// Otherwise it will print a lot of additional log informarion: per each inserted key.
// Later I will add log filtering to the perfomance test.
...
func perfomanceTest(db mdb.Storage, maxKeys int, checkKeys bool) {
